...
public class OurProductsMethodForCategoriesAndSubCategoriesRandomly {
	...
	public OurProductsMethodForCategoriesAndSubCategoriesRandomly(WebDriver driver) {

		this.driver = driver;
	}

	public void verifyCategoriesAndSortedByCategoriesRandomly(String fName, String midName, String lName, String mobNum,
			String pan) {
		...
		try {
			...
			List<WebElement> list2 = ele.findElements(
					By.xpath("//canvas[@style='position: absolute; width: 290px; height: 51px; z-index: -1;']"));

			action.click(list2.get(SortedByCategories.nextInt(totalSortedByCategories))).build().perform();
			...
			List<WebElement> list3 = ele.findElements(By.xpath(
					"(//flt-offset[contains(@style,'translate')])[15]//flt-clip-interior/flt-offset/flt-picture/flt-canvas/canvas"));

			js.executeScript("arguments[0].scrollIntoView();",
					list3.get(randCards.nextInt(totalCardsForParticularSelectedCategory)));

			action.click(list3.get(randCards.nextInt(totalCardsForParticularSelectedCategory))).build().perform();
