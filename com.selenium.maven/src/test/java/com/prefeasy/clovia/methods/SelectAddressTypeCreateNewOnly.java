...
public class SelectAddressTypeCreateNewOnly {
	...
	public SelectAddressTypeCreateNewOnly(WebDriver driver) {
		this.driver = driver;
	}

	public void selectCreateNewOptionFromDropDownWithAddressTwo(String pincode, String email, String pass, String name,
			String mob, String altMob, String add, String lndMrk, String zipCode, String adddress2) {
		...
		try {
			...
			for (String str : childWindow) {
				...
				if (!mainWindow.equalsIgnoreCase(str)) {
					...
					spci.getZipCodeFieldShipAddress().sendKeys(zipCode);

					Thread.sleep(3000);

					Select s = new Select(spci.getSelectAddressType());

					Thread.sleep(2000);

					s.selectByValue("Create New");

					spci.clickBdy();

					spci.getAddressOfCreateNewValueOfDrpDown().sendKeys(adddress2);
					
					js.executeScript("arguments[0].scrollIntoView();", spci.getSaveBtnAtShoppingAddress());
